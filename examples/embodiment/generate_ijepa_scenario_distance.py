...
DEFAULT_IJEPA_REPO = "/inspire/hdd/global_user/gongjingjing-25039/xpyu/project/ijepa"
DEFAULT_CHECKPOINT = (
    "/inspire/hdd/global_user/gongjingjing-25039/xpyu/project/ijepa/"
    "checkpoints/IN1K-vit.h.14-300e.pth.tar"
)
...
DEFAULT_OUTPUT_DIR = "rlinf/assets_isaaclab/all_setting/combined_with_ijepa_distance"
DEFAULT_STAGE_OUTPUT_DIR = "rlinf/assets_isaaclab/all_setting/curriculum_19stage_10025_ijepa"
...
def load_ijepa_encoder(
    ...
    print(f"loaded checkpoint_key={checkpoint_key} msg={msg}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compute i-JEPA RGB embeddings and reference cosine distances."
    )
    parser.add_argument("--ijepa-repo", type=Path, default=Path(DEFAULT_IJEPA_REPO))
    parser.add_argument("--checkpoint", type=Path, default=Path(DEFAULT_CHECKPOINT))
    parser.add_argument("--checkpoint-key", default="encoder")
    ...
    parser.add_argument("--stage-prefix", default="Isaaclab_19stage_10025_ijepa")
    ...
    if not args.ijepa_repo.exists():
        raise FileNotFoundError(f"i-JEPA repo not found: {args.ijepa_repo}")
    ...
    print(f"ijepa_repo={args.ijepa_repo}")
    print(f"checkpoint={args.checkpoint}")
    print(f"checkpoint_key={args.checkpoint_key}")
    ...
    if not args.checkpoint.exists():
        raise FileNotFoundError(
            f"i-JEPA checkpoint not found: {args.checkpoint}. "
            "Download it first or pass --checkpoint."
        )
    ...
    with torch.inference_mode():
        for start in range(0, len(image_paths), args.batch_size):
            ...
            batch = torch.stack([transform(image) for image in images], dim=0).to(
    ...
    distance_key = "ijepa_cosine_distance"
    embeddings_path = args.output_dir / "ijepa_embeddings_l2norm.npy"
    metadata_path = args.output_dir / "ijepa_metadata.csv"
    output_jsonl = args.output_dir / "Isaaclab_all_scenarios_10025_with_ijepa_distance.jsonl"
    summary_path = args.output_dir / "ijepa_distance_summary.json"

    np.save(embeddings_path, embedding_matrix)
    with metadata_path.open("w", encoding="utf-8", newline="") as fp:
        writer = csv.DictWriter(
            fp,
            fieldnames=["row_index", "scenario_id", "image_path", distance_key],
        )
        writer.writeheader()
    ...
    for record, distance in zip(records, distances):
        ...
        output_record["ijepa_reference_id"] = ref_id
        output_record["ijepa_checkpoint"] = str(args.checkpoint)
        output_record["ijepa_checkpoint_key"] = args.checkpoint_key
        output_record["ijepa_model_name"] = args.model_name
        output_record["ijepa_patch_size"] = args.patch_size
        output_record["ijepa_crop_size"] = args.crop_size
    ...
    summary = {
        ...
        "ijepa_repo": str(args.ijepa_repo),
