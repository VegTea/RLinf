...
MODE="both"
STAGE="all"
OUTPUT_ROOT="${REPO_PATH}/logs/curriculum_range_eval"
OUTPUT_DIR=""
SUMMARY_SOURCE="/inspire/hdd/global_user/gongjingjing-25039/xpyu/project/em_copy/projects/corriculum_rl/summarize/isaaclab_table_curriculum_incremental_scenario_ids.json"
SCENARIO_SOURCE="${REPO_PATH}/rlinf/assets_isaaclab/all_setting/combined_with_distance/Isaaclab_all_scenarios_10025_with_distance.jsonl"
TOTAL_NUM_ENVS="32"
...
usage() {
  cat <<'EOF'
Usage:
  bash examples/embodiment/eval_curriculum_ranges.sh [options] [-- extra hydra overrides...]

Purpose:
  Evaluate the original model on every curriculum range in two modes:
    cumulative  - stage N contains all scenarios with distance <= threshold_N
    incremental - stage N contains only scenarios newly added at threshold_N
...
Options:
  ...
  --output-root DIR          Parent output directory. Default: RLinf/logs/curriculum_range_eval.
  --output-dir DIR           Exact output directory. Overrides --output-root timestamp.
  --summary-source PATH      JSON made by curriculum stage summarization.
  --scenario-source PATH     Full scenario JSONL with complete scenario records.
...
Examples:
  ...
  bash examples/embodiment/eval_curriculum_ranges.sh --mode incremental --stage 0 \
...
summary_source = Path("${SUMMARY_SOURCE}")
...
for stage in summary["stages"]:
    all_ids.extend(str(sid) for sid in stage["scenario_ids"])
...
for stage in summary["stages"]:
    ...
    new_ids = [str(sid) for sid in stage["scenario_ids"]]
    cumulative_ids = previous_cumulative + new_ids
    previous_cumulative = cumulative_ids
    ...
    for mode in modes:
        ...
        metadata = {
            ...
            "range": stage["range"] if mode == "incremental" else (
                f"{summary['distance_key']} <= {stage['threshold']}"
